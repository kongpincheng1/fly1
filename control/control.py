...
class OffboardControl(Node):
    ...
    def rise_after_drop(self):
        ...
        self.fly_to_position(self.vehicle_local_position.x, self.vehicle_local_position.y, rise_target_height)
    ...
    def timer_callback(self) -> None:
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            ...
            if self.is_FinishDrop:
                if self.drop_count < 2:  # 检查是否还需要投放
                    self.rise_after_drop()
                    time.sleep(3)  # 等待上升完成（可根据实际调整时间）
                    self.is_FinishDrop = False
